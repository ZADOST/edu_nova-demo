...
class _PrincipalStudentIdCardsScreenState extends State<PrincipalStudentIdCardsScreen> {
  final StudentIdCardRepository _repository = StudentIdCardRepository();
  List<StudentIdCard> _students = [];
  ...
  Future<void> _loadStudents() async {
    final students = await _repository.fetchAllStudents();
    if (mounted) {
      setState(() {
        _students = students;
        _isLoading = false;
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              final newStudent = await _repository.addStudent(name: name, department: dept, course: course, batch: batch);
              await _loadStudents();

              _showMessage('Student ${newStudent.name} added with ID ${newStudent.uniqueCode}');
            },
